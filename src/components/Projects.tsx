  ...
  return (
    <>
      <div className="flex flex-col items-start  justify-center p-4">
        <h1 className="text-4xl font-bold mb-8 ">(some) Projects</h1>
        <div className="flex flex-col gap-8 w-full">
          {projects.map((project, index) => (
            <div key={index} className="gap-x-4 border-black dark:border-white rounded-lg flex flex-col sm:flex-row w-full py-6 px-3 shadow-lg">
              ...
              <div className="flex flex-col sm:w-2/3">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.tech.map((tech, idx) => (
                    <div key={idx} className="flex items-center text-white gap-x-2 bg-gray-700 px-3 py-1 rounded-lg">
                      <span>{tech.name}</span>
                    </div>
                  ))}
                </div>
      ...
      <Link href={"/projects"} className="text-2xl font-semibold px-5 flex items-center justify-start hover:underline mt-8">
        view all
